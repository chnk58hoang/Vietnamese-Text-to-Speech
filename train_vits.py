...
from formater.customformater import *
import argparse
...
parser.add_argument('--continue_path',help="Path to a training folder to continue training. Restore the model from the last checkpoint and continue training under the same folder."
                    ,type=str,default=None)
parser.add_argument('--sample_rate',type=int,default=22050)
parser.add_argument('--meta_filename',type=str,help='name of the metadata file')
parser.add_argument('--format',help="type of data formater"
                    ,type=int,default=2)
...
if __name__ == '__main__':
    ...
    # INITIALIZE THE DATASET, AUDIO, CHARACTER AND MODEL CONFIG
    dataset_config = BaseDatasetConfig(meta_file_train=args.meta_filename, path=args.data_path)
    audio_config = VitsAudioConfig(sample_rate=args.sample_rate, win_length=1024, hop_length=256, num_mels=80, mel_fmin=0, mel_fmax=None)

    vie_characters = VieCharacters()
    character_config = CharactersConfig(pad=vie_characters.pad,
    ...
    # init the trainer
    trainer = Trainer(
        TrainerArgs(continue_path=args.continue_path,restore_path=args.restore_path),
        config,
        args.output_path,
        model=model,
